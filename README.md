...
将本地 npm 供应链安全引擎 **`@npm-safe/core`** 迁移到 **DeepSeek Harness (dsh)** 生态的工具插件仓库。目标形态是：AI 智能体在 dsh 会话中直接调用包安全扫描工具，作为「安装前先检查」的安全门禁。引擎能力（包检查、搜索、监视、刷新、规则管理、设置、CI 门禁扫描）全部映射为 dsh 工具，共 14 个。
